...
# Usage
# Arrêter complètement l’environnement (ex. stage) :
# ./scripts/restart.sh --env stage --down-only
# 
# Redémarrer ou démarrer l’environnement (ex. stage) avec health-checks :
# ./scripts/restart.sh --env stage
# *(Utilisez --up-only si vous souhaitez démarrer sans passer par la phase d’arrêt préalable.)*
