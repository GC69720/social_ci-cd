...
if [[ ${SKIP_CHECKS} -eq 1 || -z "${CURL_BIN:-}" || -z "${SLEEP_BIN:-}" ]]; then
  msg "==> [4/4] Vérifications SKIP (flag ou curl/sleep absents)."
else
  msg "==> [4/4] Vérifications (${ENV_NAME})…"
  set +e
